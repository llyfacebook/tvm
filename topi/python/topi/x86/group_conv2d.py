...
from ..nn.conv2d import conv2d, group_conv2d_nchw, group_conv2d_nchw_impl, group_conv2d_NCHWc, \
...
def _fallback_schedule(cfg, wkl):
    ...
    wkl : topi.nn.group_conv2d.Workload
...
@autotvm.register_topi_compute(group_conv2d_NCHWc, 'cpu', 'direct')
def _group_conv2d_NCHWc_cpu(cfg, data, kernel, strides, padding, dilation,
                                layout, out_layout, out_dtype=None):
    out_dtype = data.dtype if out_dtype is None else out_dtype
    batch, in_channel_chunk, in_height, in_width, in_channel_block = get_const_tuple(data.shape)
    out_channel_chunk, in_channel_per_group_chunk, filter_height, filter_width, _, out_channel_block \
    ...
    in_channel_per_group = in_channel_per_group_chunk * in_channel_block
    groups = in_channel // in_channel_per_group
    ...
    wkl = _get_conv2d_workload(tvm.placeholder((batch, in_channel, in_height, in_width), dtype=data.dtype),
                        tvm.placeholder((out_channel, in_channel_per_group, filter_height, filter_width),
                                        dtype=kernel.dtype),
    ...
    ic_outer = tvm.reduce_axis((0, in_channel_chunk//groups), name='ic_outer')
    ic_inner = tvm.reduce_axis((0, in_channel_block), name='ic_inner')
    Output = tvm.compute(
        (batch, out_channel_chunk, out_height, out_width, out_channel_block),
        lambda b, oco, oh, ow, oci: tvm.sum(
            (data_pad[b, (oco*out_channel_block//(out_channel_chunk*out_channel_block//groups))*(in_channel_chunk//groups) + ic_outer,
                oh*HSTR+kh, ow*WSTR+kw,
                ic_inner].astype(out_dtype) *
            kernel[oco, ic_outer, kh, kw, ic_inner, oci].astype(out_dtype)),
            axis=[kh, kw, ic_outer, ic_inner]),
        name='GroupConv2d', tag="group_conv2d_NCHWc")
    return Output


@autotvm.register_topi_schedule(generic.schedule_group_conv2d_NCHWc, 'cpu', ['direct'])
def schedule_group_conv2d_NCHWc(cfg, outs):
    """CPU schedule for group conv2d in NCHW[x]c layout"""
        ...
        if tag.is_broadcast(op.tag):
            ...
            for tensor in op.input_tensors:
                if tensor.op.input_tensors and tensor.op not in scheduled_ops:
                    traverse(tensor.op)
        if 'group_conv2d_NCHWc' in op.tag:
            conv_out = op.output(0)
            data = conv_out.op.input_tensors[0]
            kernel = conv_out.op.input_tensors[1]
            _schedule_group_conv2d_NCHWc_impl(s, cfg, data, kernel, conv_out, outs[0])
        scheduled_ops.append(op)
    traverse(outs[0].op)
    return s

def _schedule_group_conv2d_NCHWc_impl(s, cfg, data, kernel, conv_out, output):
    ...
    kh, kw, ic_outer, ic_inner = s[CC].op.reduce_axis
    ow_chunk, ow_block = s[CC].split(ow, factor=tile_ow)
    s[CC].reorder(ic_chunk, ic_outer, oh, kh, kw, ow_block, ic_inner, ic_block)
...
@autotvm.task.register("topi_x86_group_conv2d_NCHWc")
def _topi_nn_group_conv2d_NCHWc(*args, **kwargs):
    assert not kwargs, "Do not support kwargs in template function call"
    data, kernel, strides, padding, dilation, group, dtype = deserialize_args(args)

    batch, in_channel, height, width = get_const_tuple(data.shape)
    out_channel, in_channel_per_group, kh, kw = get_const_tuple(kernel.shape)
    ...
    new_kernel_shape = (out_channel // oc_bn, in_channel_per_group // ic_bn, kh, kw, ic_bn, oc_bn)
    ...
    C = _group_conv2d_NCHWc_cpu(cfg, new_data, new_kernel, strides, padding, dilation,
                                    data_layout, out_layout, dtype)
    s = schedule_group_conv2d_NCHWc(cfg, [C])
